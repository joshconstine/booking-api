...
func GetSettingsForRental(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&setting.ID, &setting.RentalUnitID, &setting.NightlyCost, &setting.MinimumBookingDuration, &setting.AllowsPets, &setting.CleaningFee, &setting.CheckInTime, &setting.CheckOutTime); err != nil {
