...
func buildServer(env config.EnvVars) (*chi.Mux, func(), error) {
	...
	bookingDetailsService := services.NewBookingDetailsServiceImplementation(bookingDetailsRepository, bookingPaymentRepository, bookingService)
	...
	rentalController := controllers.NewRentalController(rentalService, amenityService)
	...
	accountController := controllers.NewAccountController(bookingCostItemService, bookingPaymentService, accountRepository)
